...
public class HubPathController {
	...
	/** 허브 간 이동정보 등록 POST /api/hub-path */
	@Operation(
			summary = "허브 경로 생성",
				...
				content =
						@Content(
								schema =
										@Schema(
												implementation =
														com.fastline.hubservice.presentation.response.HubPathCreateResponse
																.class))),
		@io.swagger.v3.oas.annotations.responses.ApiResponse(
				responseCode = "400",
				description = "유효성 오류"),
	...
	public ResponseEntity<ApisResponse<HubPathCreateResponse>> createHubRoute(
		...
		HubPathCreateCommand command =
				HubPathCreateCommand.builder()
	...
	@Operation(summary = "허브 경로 단건 조회", description = "hubPathId로 단건 허브 경로를 조회합니다.")
	@io.swagger.v3.oas.annotations.responses.ApiResponses({
		@io.swagger.v3.oas.annotations.responses.ApiResponse(
				responseCode = "200",
				description = "조회 성공",
