...
public class Hub extends TimeBaseEntity  {
    ...
    /**
     * 허브 생성 팩토리 메서드
     *  - 애그리거트 불변식(invariants) 검사
     *  - 외부 레이어의 Command/DTO에 의존하지 않음 (도메인 순수성 유지)
     *
     * 주의: Application/Web 레이어의 "CreateHubCommand" 같은 DTO를 여기로 넘기지 마세요.
     * 도메인 계층 내부의 명세 객체(Spec/VO)만 받도록 하여 계층 간 결합을 방지합니다.
     */
    /**
     * 도메인 내부 응집을 위한 생성 스펙
     * 외부 레이어(Command/DTO)에 의존하지 않도록 Hub 내부에 캡슐화한다.
     */
