...
/**
 * HubPath JPA 어댑터 - 인프라스트럭처 계층에서 Spring Data JPA 리포지토리를 감싸 도메인 친화적 API 제공 - 소프트 삭제(deletedAt) 조건을
 * 기본으로 하는 편의 메서드 노출
 */
@Repository
...
public class HubPathJpaAdapter implements HubPathRepository {
	...
	@Override
	public Optional<HubPath> findByStartHub_HubIdAndEndHub_HubIdAndDeletedAtIsNull(
			UUID startHubId, UUID endHubId) {
		return hubPathJpaRepository.findByStartHub_HubIdAndEndHub_HubIdAndDeletedAtIsNull(
				startHubId, endHubId);
	}
