...
public interface HubPathJpaRepository extends JpaRepository<HubPath, UUID> {
	...
	@Query(
			"select hp from HubPath hp where hp.hubPathId = :id and hp.deletedAt is null and hp.active = true")
	...
	@Query(
			"select hp from HubPath hp where hp.startHub.hubId = :startId and hp.endHub.hubId = :endId and hp.deletedAt is null and hp.active = true")
	Optional<HubPath> findActiveByStartAndEnd(
			@Param("startId") UUID startId, @Param("endId") UUID endId);
}
