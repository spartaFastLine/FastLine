...
public class HubPathService {
	...
	@Transactional
	@CacheEvict(
			value = {"hubGraph", "route"},
			allEntries = true) // 경로 변경 시 그래프/경로 캐시 무효화
	public HubPathCreateResponse createRoute(HubPathCreateCommand cmd) {
		...
		try {
			...
			log.warn(
					"Unexpected error while calling Naver API. Falling back to request metrics. reason={}",
					e.getMessage());
		}
	...
	private HubPathCreateResponse validateAndUpsertWithMetrics(
			Hub start, Hub end, Long distance, LocalTime duration, Boolean activeFlag) {
	...
	private HubPathCreateResponse upsertEdgeWithMetrics(
			Hub start, Hub end, Long distance, LocalTime duration, Boolean activeFlag) {
		HubPath edge =
				hubPathRepository
						.findActiveByStartAndEnd(start.getHubId(), end.getHubId())
						.orElseGet(() -> HubPath.builder().startHub(start).endHub(end).active(true).build());
		...
		for (var entry : g.getAdj().entrySet()) {
			...
			for (Graph.Edge e : entry.getValue()) {
				if (Double.isInfinite(e.getDistance()) || e.getDistance() == 0.0) {
					hubPathRepository
							.findActiveByStartAndEnd(from, e.getTo())
							.ifPresent(
									leg -> {
										// 좌표
										...
										try {
											...
											if (stopovers != null) {
												...
												if (sumSec > 0)
													leg.setDuration(LocalTime.ofSecondOfDay(sumSec % (24 * 3600)));
												hubPathRepository.save(leg);
											}
										} catch (Exception ex) {
											// 실패 시 로깅만; 다음 호출/배치에서 보정
											log.warn(
													"edge metric fill failed from {} to {}: {}",
													from,
													e.getTo(),
													ex.getMessage());
										}
